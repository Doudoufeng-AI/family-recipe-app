    ...
    <div class="auth-content">
      <div class="auth-logo">
        <svg viewBox="0 0 64 64" fill="none" class="logo-svg">
          ...
          <path d="M28 28H36" stroke="#34C759" stroke-width="2.5" stroke-linecap="round"/>
          <path d="M28 33H34" stroke="#34C759" stroke-width="2.5" stroke-linecap="round"/>
        </svg>
      </div>
      <h1 class="auth-title">创建账号</h1>
      <p class="auth-sub">加入家庭，共享美味</p>

      <div class="auth-form">
        <div class="input-group">
          <input class="ios-input" v-model="form.nickname" placeholder="昵称（如：妈妈、小李）" />
        </div>
        <div class="input-group">
          <input class="ios-input" v-model="form.username" placeholder="用户名（登录用）" />
        </div>
        <div class="input-group">
          <input class="ios-input" v-model="form.password" type="password" placeholder="密码" />
        </div>
        <p v-if="error" class="err">{{ error }}</p>
        <button class="btn btn-primary btn-block mt-16" @click="onRegister" :disabled="loading">
          {{ loading ? '注册中...' : '注册' }}
        </button>
        <p class="auth-switch">
...
<style scoped>
.auth-page { min-height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 20px; background: var(--bg); }
...
.ios-input { width: 100%; padding: 14px 16px; border: none; border-radius: 14px; font-size: 16px; outline: none; background: #fff; box-shadow: 0 0 0 1px var(--separator); -webkit-appearance: none; transition: box-shadow .15s; }
